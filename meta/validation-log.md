...
- [ok] "NO backend reserved-name list in tinycloud-node" — cx(js-sdk, mounted tinycloud-node): no `SYSTEM_SPACES` array; `tinycloud-auth/src/resource.rs::Name` accepts any string (TODO validation); only `public` is special, via `tinycloud-node-server/src/routes/public.rs::is_public_space`. Upgraded from unverified.
- [ok] Exact SDK constants — cx: `ACCOUNT_REGISTRY_SPACE="account"`, `ACCOUNT_REGISTRY_PATH="applications/"`, `SECRETS_SPACE="secrets"`, `DEFAULT_MANIFEST_SPACE="applications"`, `ENCRYPTION_MANIFEST_SPACE="encryption"` (manifest.ts); secrets `resolveSecretPath` → `vault/secrets/<NAME>` (paths.ts); network `buildNetworkId` → `urn:tinycloud:encryption:{ownerDid}:{name}` (networkId.ts).
