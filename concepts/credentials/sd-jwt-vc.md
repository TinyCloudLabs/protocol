...
- **Structure** — `<issuer-JWT>~<disclosure_1>~…~<disclosure_n>~[<key-binding-JWT>]`. The issuer JWT is a normal 3-segment compact JWT signed by the [[witness-service|witness]] (`did:web:issuer.credentials.org`, EdDSA / Ed25519). Each disclosure is base64url of a JSON array `[salt, claimName, value]`; the signed payload holds only the *digests* of these, so a verifier confirms a disclosed claim by hashing the disclosure and matching the digest.
